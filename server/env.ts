...
export class Environment {
  ...
  /**
   * Default user role assigned to new users signing in via SSO providers (OIDC,
   * Google, Slack, etc.) when the authentication provider does not specify a
   * role. Overrides the team-level defaultUserRole setting. Must be "viewer" or
   * "member".
   */
  @IsOptional()
  @IsIn(["viewer", "member"])
  public DEFAULT_USER_ROLE = this.toOptionalString(
    environment.DEFAULT_USER_ROLE
  );
